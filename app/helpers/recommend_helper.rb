  ...
  def recommend_etc(item)
    ...
    if session[:id].nil?
      ...
      if item.is_a? Note
        ...
        r += link_to "#{t('recommend')}#{item.recommend_count==0 ? '' : '(' + item.recommend_count.to_s + ')' }", 'javascript:;', :id => "recommend_note_#{item.id}", :onclick => onclick
        #        else
        ...
        r += link_to "#{t('recommend')}#{item.recommend_count==0 ? '' : '(' + item.recommend_count.to_s + ')' }", 'javascript:;', :id => "recommend_photo_#{item.id}", :onclick => onclick
        #        else
        #          r += link_to t('cancel_recommend'), "##{item.id}", :id => "recommend#{item.id}", :onclick => onclick
        #        end
      elsif item.is_a? Blog
        r += link_to "#{t('recommend')}#{item.recommend_count==0 ? '' : '(' + item.recommend_count.to_s + ')' }", 'javascript:;', :id => "recommend_blog_#{item.id}", :onclick => onclick
